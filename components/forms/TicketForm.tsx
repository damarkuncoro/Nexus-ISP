...
export const TicketForm: React.FC<TicketFormProps> = ({ 
  ...
  // Helper to calculate Due Date based on SLA hours
  ...
  useEffect(() => {
    if (initialData) {
      ...
      // Default to first category if existing data has no category (rare) or specific one
      ...
      // Handle existing Due Date conversion to local datetime string
  ...
  const handleCategoryChange = (newCode: string) => {
      ...
      // Auto-update Due Date based on SLA when category changes
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSubmit({ 
        ...
        // Convert local datetime string back to ISO for API
  ...
  // Get current category config for UI display
            ...
            <CardContent className="p-8">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                    ...
                    <div className="space-y-6 lg:pl-8 lg:border-l lg:border-gray-100">
                            ...
                            <div className="flex justify-between items-center mb-1">
                                ...
                                {currentCategoryConfig && (
                                    <span className="text-[10px] bg-blue-50 text-blue-700 px-1.5 py-0.5 rounded font-medium border border-blue-100">
                                        SLA: {currentCategoryConfig.sla_hours}h
                                    </span>
                                )}
                            </div>
                            <Select
                        ...
                        <div>
                            <Label htmlFor="dueDate" className="mb-1 flex items-center gap-2">
                                Due Date / SLA
                            ...
                            <p className="text-[10px] text-gray-400 mt-1">
                                Auto-calculated based on Category SLA.
                            </p>
                        </div>
                    </div>
                </div>

                {/* Resolution Notes & RCA (Visible if Resolved/Closed or Edit Mode) */}
                <div className={`mt-8 transition-all duration-500 ease-in-out ${status === TicketStatus.RESOLVED || status === TicketStatus.VERIFIED || status === TicketStatus.CLOSED ? 'block' : 'hidden'}`}>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="bg-purple-50 border border-purple-100 rounded-lg p-6">
                            <Label htmlFor="rca" className="text-purple-800 mb-2 flex items-center">
                            ...
                            <Textarea
                                ...
                                placeholder="Why did this issue occur? (e.g. Fiber cut, Power outage)"
                            />
                        </div>
                    </div>
                </div>
