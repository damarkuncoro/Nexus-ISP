...
import { DollarSign, Search, Filter, Calendar, CheckCircle2, AlertCircle, Clock, PlayCircle, FileText, Download } from 'lucide-react';
...
export const FinanceView: React.FC<FinanceViewProps> = ({ customers, plans, currency }) => {
  ...
  const handleDownload = (invoice: any) => {
      // Reconstruct minimal customer object if joined data is missing full details
      const customer = {
          name: invoice.customer?.name || 'Unknown',
          email: invoice.customer?.email || '',
          company: invoice.customer?.company || '',
          // other fields optional for invoice generator
      } as Customer;
