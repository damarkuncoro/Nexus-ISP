    ...
    <div className="space-y-6 animate-in fade-in duration-500">
        <Grid cols={1} className="md:grid-cols-3" gap={6}>
            <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex items-center justify-between">
                <div><p className="text-sm text-gray-500 font-medium">Total SKU</p><p className="text-3xl font-bold text-gray-900 mt-1">{stats.totalItems}</p></div>
                <div className="p-3 bg-blue-50 rounded-lg text-blue-600"><Package className="w-6 h-6" /></div>
            </div>
            <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex items-center justify-between">
                <div><p className="text-sm text-gray-500 font-medium">Low Stock Alerts</p><p className="text-3xl font-bold text-amber-600 mt-1">{stats.lowStock}</p></div>
                <div className="p-3 bg-amber-50 rounded-lg text-amber-600"><AlertTriangle className="w-6 h-6" /></div>
            </div>
            <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex items-center justify-between">
                <div><p className="text-sm text-gray-500 font-medium">Total Valuation</p><p className="text-3xl font-bold text-green-600 mt-1">{formatCurrency(stats.totalValue, currency)}</p></div>
                <div className="p-3 bg-green-50 rounded-lg text-green-600"><TrendingUp className="w-6 h-6" /></div>
            </div>
        </Grid>

        <div className="space-y-4">
            <Flex className="p-4 bg-white rounded-xl border border-gray-200 shadow-sm flex-col sm:flex-row gap-4 justify-between items-center">
                <Flex gap={4} className="w-full sm:w-auto flex-1">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        <select className="h-10 rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-primary-500" value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)}>
            ...
            {filteredItems.length > 0 ? (
                viewMode === 'table' ? (
                                        ...
                                        <TableRow key={item.id} className="group">
                                            ...
                                            <TableCell>
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-sm font-medium text-gray-900 ml-1">{item.quantity} {item.unit}</span>
                                                </div>
                                            </TableCell>
                                            ...
                                            <TableCell className="text-right">
                                                {hasPermission('manage_network') && (
                                                    <Flex justify="end" gap={1} className="opacity-0 group-hover:opacity-100 transition-opacity">
                                                        <Button variant="ghost" size="icon" onClick={() => onEditItem(item)}><Edit2 className="w-4 h-4 text-gray-500" /></Button>
                                                        <AlertDialog>
                    ...
                    <Grid cols={1} className="sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" gap={6}>
                        {filteredItems.map(item => {
                            ...
                            const statusColor = isOutOfStock ? 'border-red-200 bg-red-50' : isLowStock ? 'border-amber-200 bg-amber-50' : 'border-gray-200 bg-white';

                            return (
                                <Card key={item.id} className={`hover:shadow-md transition-all ${isLowStock || isOutOfStock ? 'border-2' : ''} ${statusColor}`}>
                                    <CardContent className="p-5">
                                        <Flex justify="between" align="start" className="mb-3">
                                            <Badge variant="secondary" className="bg-white/80">{item.category}</Badge>
                                            {hasPermission('manage_network') && (
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end">
                                                        ...
                                                        <AlertDialog>
                                                            <AlertDialogTrigger asChild>
                                                                <div className="relative flex cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none transition-colors hover:bg-red-50 hover:text-red-600 text-red-600 w-full">
                                        ...
                                        <div className="mb-4">
                                            <h4 className="text-lg font-bold text-gray-900 line-clamp-2" title={item.name}>{item.name}</h4>
                                        ...
                                        <div className="bg-white/60 rounded-lg p-3 border border-black/5">
                                            <Flex justify="between" align="end">
                                                <div>
                                                    ...
                                                    <p className={`text-2xl font-bold ${isOutOfStock ? 'text-red-600' : isLowStock ? 'text-amber-600' : 'text-gray-900'}`}>
                                                        {item.quantity} <span className="text-sm font-medium text-gray-500">{item.unit}</span>
                                                    </p>
                                                </div>
                                                <div className="text-right">
                                                    <p className="text-xs text-gray-500 uppercase font-medium">Value</p>
                                                    <p className="text-sm font-mono font-medium text-gray-900">{formatCurrency(item.cost_price, currency)}</p>
                                                </div>
                                            </Flex>
                                        </div>

                                        <div className="mt-4 pt-3 border-t border-black/5 flex items-center justify-between text-xs text-gray-500">
                ...
                <div className="p-12 bg-gray-50 rounded-xl border border-gray-200 border-dashed">
