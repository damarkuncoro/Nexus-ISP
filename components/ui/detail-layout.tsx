...
interface PageHeaderProps {
  ...
  onBack: () => void;
  actions?: ReactNode;
  children?: ReactNode; // For breadcrumbs or extra info
}
...
interface DetailHeroProps {
  ...
  avatarUrl?: string;
  metadata?: ReactNode; // Secondary info row (Email, Phone, ID)
  className?: string;
  roleIcon?: React.ElementType; // Icon inside avatar if image missing
}
...
// --- 4. Info Section (Grid of Details) ---
export const DetailSection: React.FC<{ title: string, icon?: React.ElementType, children: ReactNode, action?: ReactNode, className?: string }> = ({ title, icon: Icon, children, action, className }) => (
...
// --- 5. Info Item (Single Row/Block) ---
export const InfoItem: React.FC<{ icon: React.ElementType, label: string, value?: ReactNode, className?: string }> = ({ icon: Icon, label, value, className }) => (
