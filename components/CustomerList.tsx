...
export const CustomerList: React.FC<CustomerListProps> = ({ customers, onDelete, onSelect, initialSearch = '' }) => {
  ...
  const columns = [
      { id: InstallationStatus.PENDING_SURVEY, label: 'Pending Survey', icon: MapPin, color: 'bg-purple-50 border-purple-200 text-purple-700' },
      { id: InstallationStatus.SURVEY_COMPLETED, label: 'Survey Done', icon: CheckCircle2, color: 'bg-blue-50 border-blue-200 text-blue-700' },
      { id: InstallationStatus.SCHEDULED, label: 'Scheduled', icon: Calendar, color: 'bg-amber-50 border-amber-200 text-amber-700' },
      { id: InstallationStatus.INSTALLED, label: 'Installed', icon: HardHat, color: 'bg-green-50 border-green-200 text-green-700' },
      { id: InstallationStatus.SURVEY_FAILED, label: 'Issues / Failed', icon: XCircle, color: 'bg-red-50 border-red-200 text-red-700' },
  ];
  ...
  return (
    <div className="space-y-6">
        {/* Header Controls */}
        <Flex align="center" justify="between" className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex-col sm:flex-row gap-4">
            ...
            <div className="bg-gray-100 p-1 rounded-lg flex items-center shrink-0">
                <button 
                    ...
                    className={`p-2 rounded-md transition-all flex items-center gap-2 text-sm font-medium ${viewMode === 'list' ? 'bg-white shadow text-gray-900' : 'text-gray-500 hover:text-gray-900'}`}
                >
                    <LayoutList className="w-4 h-4" /> List
                </button>
                <button 
                    onClick={() => setViewMode('board')} 
                    className={`p-2 rounded-md transition-all flex items-center gap-2 text-sm font-medium ${viewMode === 'board' ? 'bg-white shadow text-gray-900' : 'text-gray-500 hover:text-gray-900'}`}
                >
                ...
                <div className="overflow-x-auto pb-4">
                    <Flex gap={4} className="min-w-[1200px] h-[calc(100vh-220px)] items-stretch">
                        {columns.map(col => {
                            ...
                            return (
                                <div key={col.id} className={`flex-1 min-w-[280px] flex flex-col rounded-xl border bg-gray-50/50 ${col.color.replace('text', 'border')}`}>
                                    {/* Column Header */}
                                    <div className={`p-3 border-b flex justify-between items-center rounded-t-xl bg-white ${col.color.replace('bg', 'border')}`}>
                                        <div className="flex items-center gap-2">
                                            <Icon className={`w-4 h-4`} />
                                            <span className="font-semibold text-sm">{col.label}</span>
                                        </div>
                                        <Badge variant="secondary" className="bg-gray-100 text-gray-600 border-0">{items.length}</Badge>
                                    </div>
                                    ...
                                    <div className="p-2 flex-1 overflow-y-auto space-y-2">
                                        {items.map(customer => (
                                            <Card 
                                                key={customer.id} 
                                                onClick={() => onSelect && onSelect(customer)}
                                                className="cursor-pointer hover:shadow-md transition-all border-gray-200 group relative bg-white"
                                            >
                                                <CardContent className="p-3">
                                                    <div className="flex justify-between items-start mb-2">
                                                        <div className="flex items-center gap-1.5">
                                                            ...
                                                            <span className="text-xs font-medium text-gray-500 uppercase">{customer.type}</span>
                                                        </div>
                                                        <CustomerStatusBadge status={customer.account_status} />
                                                    </div>
                                                    
                                                    <h4 className="font-bold text-gray-900 text-sm mb-1 line-clamp-1">{customer.name}</h4>
                                                    ...
                                                    <div className="pt-2 border-t border-gray-50 flex justify-between items-center mt-2">
                                                        {customer.subscription_plan ? (
                                                            <span className="text-xs font-medium text-primary-600 bg-primary-50 px-1.5 py-0.5 rounded">{customer.subscription_plan}</span>
                                                        ) : (
                                        ...
                                        {items.length === 0 && (
                                            <div className="text-center py-8 px-4 border-2 border-dashed border-gray-200 rounded-lg">
                                                <p className="text-xs text-gray-400">No subscribers in this stage</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
