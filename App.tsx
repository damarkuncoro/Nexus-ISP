...
export const App: React.FC = () => {
  ...
  const renderContent = () => {
    ...
    switch (view) {
        ...
        case 'customers': return selectedCustomer ? <CustomerDetail customer={selectedCustomer} tickets={tickets.filter(t => t.customer_id === selectedCustomer.id)} onBack={() => setSelectedCustomer(null)} onTicketEdit={openEditTicket} onTicketDelete={handleDeleteTicket} currency={currency} onPlanClick={handlePlanClick} onCreateTicket={() => openCreateTicket(selectedCustomer)} onTicketClick={handleTicketClick} devices={devices.filter(d => d.customer_id === selectedCustomer.id)} onAddDevice={() => openCustomerDeviceForm(selectedCustomer.id)} onEditDevice={(d) => openCustomerDeviceForm(selectedCustomer.id, d)} onDeleteDevice={handleDeleteDevice} plans={plans} onUpdateCustomer={handleUpdateCustomer} /> : <CustomerList customers={customers} onDelete={handleDeleteCustomer} onSelect={setSelectedCustomer} initialSearch={customerSearch} />;
    ...
    <Layout currentView={getActiveNav()} onViewChange={(v) => handleViewChange(v as AppView)}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          ...
          {shouldShowAddButton() && (
            <button onClick={() => { if (view === 'customers') openCreateCustomer(); else if (view === 'plans') openCreatePlan(); else if (view === 'employees') openCreateEmployee(); else if (view === 'network') openCreateDevice(); else openCreateTicket(); }} className="inline-flex items-center px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white text-sm font-medium rounded-lg shadow-sm">
              <Plus className="w-5 h-5 mr-2" />
              {view === 'customers' ? 'New Customer' : view === 'plans' ? 'New Plan' : view === 'employees' ? 'New Member' : view === 'network' ? 'New Device' : 'New Ticket'}
            </button>
          )}
        </div>
        {renderContent()}
