...
import { LayoutDashboard, Ticket as TicketIcon, Settings, Menu, X, Bell, Users, Wifi, Server, Briefcase, ChevronDown, User, Shield, Package, CircleDollarSign, Search, Map, Moon, Sun, BookOpen, BarChart3 } from 'lucide-react';
...
interface LayoutProps {
  ...
  currentView: 'dashboard' | 'tickets' | 'customers' | 'plans' | 'network' | 'settings' | 'employees' | 'alerts' | 'inventory' | 'finance' | 'map' | 'kb' | 'reports';
  onViewChange: (view: 'dashboard' | 'tickets' | 'customers' | 'plans' | 'network' | 'settings' | 'employees' | 'alerts' | 'inventory' | 'finance' | 'map' | 'kb' | 'reports') => void;
}
                  ...
                  <div className="relative">
                    ...
                    {isProfileMenuOpen && (
                        <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-gray-100 dark:border-slate-700 py-2 z-50">
                            ...
                            <button onClick={() => handleRoleSwitch(EmployeeRole.ADMIN)} className="w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-slate-700 flex items-center gap-2">
                                <Shield className="w-4 h-4 text-purple-600" /> Admin
                            </button>
                            <button onClick={() => handleRoleSwitch(EmployeeRole.MANAGER)} className="w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-slate-700 flex items-center gap-2">
                                <Briefcase className="w-4 h-4 text-blue-600" /> Manager
                            </button>
                            <button onClick={() => handleRoleSwitch(EmployeeRole.SUPPORT)} className="w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-slate-700 flex items-center gap-2">
                                <Users className="w-4 h-4 text-green-600" /> Support
                            </button>
                            <button onClick={() => handleRoleSwitch(EmployeeRole.TECHNICIAN)} className="w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-slate-700 flex items-center gap-2">
