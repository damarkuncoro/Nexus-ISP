...
import { Copy, Check, Database, Server, ShieldCheck, Globe, Loader2, Tag, Building2, FileText } from 'lucide-react';
        ...
        <TabsContent value="database">
            <Grid cols={1} className="lg:grid-cols-2" gap={6}>
                <Card>
                    ...
                    <CardContent>
                        <Flex align="center" gap={3} className="p-4 bg-gray-50 dark:bg-slate-800 rounded-lg border border-gray-100 dark:border-slate-700">
                            <div className={`w-3 h-3 rounded-full ${connectionStatus === 'connected' ? 'bg-green-500' : 'bg-red-500'}`} />
                            <div>
                                <p className="font-medium text-gray-900 dark:text-white capitalize">{connectionStatus}</p>
                                <p className="text-xs text-gray-500 dark:text-gray-400 break-all">{SUPABASE_URL}</p>
                            </div>
                        </Flex>
                    </CardContent>
                </Card>
                ...
                {hasPermission('manage_settings') && (
                    <Card className="lg:col-span-2">
                        ...
                        <CardContent>
                            ...
                            <div className="mt-4 p-4 bg-yellow-50 dark:bg-yellow-900/20 text-yellow-800 dark:text-yellow-200 rounded-lg border border-yellow-200 dark:border-yellow-800 flex gap-3">
